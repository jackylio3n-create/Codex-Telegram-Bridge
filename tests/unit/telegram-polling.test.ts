...
test("telegram polling stop aborts an in-flight getUpdates request", async () => {
  ...
  const polling = new TelegramPollingService(client, {
    channelOffsets: {
      get() {
        return null;
      },
    ...
    telegramUserAuth: {
      ...
      getOrCreateFirstSeen() {
        throw new Error("getOrCreateFirstSeen should not be called for an aborted poll.");
      },
      markVerified() {
        throw new Error("markVerified should not be called for an aborted poll.");
      },
      setPreferredLanguage() {
        throw new Error("setPreferredLanguage should not be called for an aborted poll.");
      },
      recordFailedAttempt() {
        throw new Error("recordFailedAttempt should not be called for an aborted poll.");
      }
    }
  }, {
    botToken: "token",
